package org.raystack.dagger.functions.udfs.aggregate.accumulator;

import com.google.protobuf.ByteString;
import org.raystack.dagger.functions.udfs.aggregate.feast.handler.ValueEnum;
...
public class FeatureWithTypeAccumulatorTest {
    ...
    public void shouldHandleAllValueTypesWithFeatureTypeUdf() {
        ...
        data.put("FloatKey", new Tuple2<>(1.0F, ValueEnum.FloatType));
        data.put("StringKey", new Tuple2<>("stringValue", ValueEnum.StringType));
        data.put("DoubleKey", new Tuple2<>(1.00D, ValueEnum.DoubleType));
        data.put("IntegerKey", new Tuple2<>(1, ValueEnum.IntegerType));
        data.put("LongKey", new Tuple2<>(1L, ValueEnum.LongType));
        data.put("ByteKey", new Tuple2<>(ByteString.copyFrom("value1".getBytes()), ValueEnum.ByteType));
        data.put("BoolKey", new Tuple2<>(true, ValueEnum.BooleanType));
        data.put("TimestampKey", new Tuple2<>(getTimestampAsRow(123141, 431231), ValueEnum.TimestampType));
    ...
    public void shouldPopulateIdAndNameWithSameValuesInFeatureRowWithFeatureTypeUdf() {
        FeatureWithTypeAccumulator featureAccumulator = new FeatureWithTypeAccumulator();
        featureAccumulator.add("FloatKey", 1.0F, ValueEnum.FloatType);
    ...
    public void shouldPopulateAndRemoveKeysInFeatureRowWithFeatureTypeUdf() {
        FeatureWithTypeAccumulator featureAccumulator = new FeatureWithTypeAccumulator();
        featureAccumulator.add("FloatKey", 1.0F, ValueEnum.FloatType);
        featureAccumulator.add("keyToRemove", 1.0F, ValueEnum.FloatType);
        featureAccumulator.remove("keyToRemove", 1.0F, ValueEnum.FloatType);
    ...
    public void shouldPopulateDuplicateKeysWithDifferentValuesInFeatureRowWithFeatureTypeUdf() {
        FeatureWithTypeAccumulator featureAccumulator = new FeatureWithTypeAccumulator();
        HashMap<String, Tuple2<Object, ValueEnum>> data = new HashMap<>();
        data.put("duplicateKey", new Tuple2<>(1.0F, ValueEnum.FloatType));
        data.put("duplicateKey", new Tuple2<>(2.0F, ValueEnum.FloatType));
        populateFeatureAccumulator(featureAccumulator, data);
