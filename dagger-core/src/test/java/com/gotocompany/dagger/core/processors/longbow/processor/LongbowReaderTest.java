...
import com.gotocompany.dagger.core.processors.longbow.range.LongbowAbsoluteRange;
...
public class LongbowReaderTest {
    ...
    public void setup() {
        initMocks(this);
        when(configuration.getString(Constants.PROCESSOR_LONGBOW_GCP_PROJECT_ID_KEY, Constants.PROCESSOR_LONGBOW_GCP_PROJECT_ID_DEFAULT)).thenReturn("test-project");
        when(configuration.getString(Constants.PROCESSOR_LONGBOW_GCP_INSTANCE_ID_KEY, Constants.PROCESSOR_LONGBOW_GCP_INSTANCE_ID_DEFAULT)).thenReturn("test-instance");
        when(configuration.getString(Constants.DAGGER_NAME_KEY, Constants.DAGGER_NAME_DEFAULT)).thenReturn("test-job");
    ...
    public void shouldCaptureExceptionWithStatsDManagerOnReadDocumentFailure() throws Exception {
        ...
        verify(meterStatsManager, times(1)).markEvent(LongbowReaderAspects.FAILED_ON_READ_DOCUMENT);
    ...
    public void shouldHandleClose() throws Exception {
        ...
        verify(meterStatsManager, times(1)).markEvent(LongbowReaderAspects.CLOSE_CONNECTION_ON_READER);
    }
    ...
    public void shouldFailOnTimeout() throws Exception {
        ...
        verify(meterStatsManager, times(1)).markEvent(LongbowReaderAspects.TIMEOUTS_ON_READER);
