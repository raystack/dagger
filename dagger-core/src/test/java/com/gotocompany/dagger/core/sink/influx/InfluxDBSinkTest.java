...
public class InfluxDBSinkTest {
    ...
    public void setUp() throws Exception {
        initMocks(this);
        when(configuration.getString(Constants.SINK_INFLUX_URL_KEY, Constants.SINK_INFLUX_URL_DEFAULT)).thenReturn("http://localhost:1111");
        when(configuration.getString(Constants.SINK_INFLUX_USERNAME_KEY, Constants.SINK_INFLUX_USERNAME_DEFAULT)).thenReturn("usr");
        when(configuration.getString(Constants.SINK_INFLUX_PASSWORD_KEY, Constants.SINK_INFLUX_PASSWORD_DEFAULT)).thenReturn("pwd");
        ...
        when(configuration.getInteger(Constants.SINK_INFLUX_FLUSH_DURATION_MS_KEY, Constants.SINK_INFLUX_FLUSH_DURATION_MS_DEFAULT)).thenReturn(1000);
